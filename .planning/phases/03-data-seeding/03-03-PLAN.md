...
must_haves:
  truths:
    ...
    - "Hero images exist for each major origin region (16 HoneyOrigin values)"
...
4. **Estimated cost**: ~$2.10 for generating all images (200+ honeys + 50+ sources + 17 type heroes + 16 origin heroes)
  </instructions>
...
const prompts = {
  ...
  heroOriginRegion: (origin, displayName) => {
...
1. Generate hero images for each FloralSource (17 images)
   - Save to honeys/heroes/floral/{floralSource}.webp
2. Generate hero images for each HoneyOrigin (16 images)
   - Save to honeys/heroes/origin/{origin}.webp
   - Use heroOriginRegion prompt template
3. For each honey in honeys.json:
...
4. For each local source in local-sources.json:
   - Generate source image
   - Save to sources/{slug}.webp
   - Save to sources/thumbnails/{slug}.webp
   - Update JSON entry with real URLs
5. Write updated JSON files back to disk
...
Verify dependencies install successfully.
  ...
  <done>Image generation script created with Replicate + R2 integration, including both floral source and origin region hero prompts</done>
</task>
...
This will:
1. Generate 17 hero images for FloralSource types (CLOVER, WILDFLOWER, MANUKA, etc.)
2. Generate 16 hero images for HoneyOrigin regions (USA, NEW_ZEALAND, AUSTRALIA, etc.)
3. Generate ~200+ honey product images
4. Generate ~50+ local source images
5. Generate thumbnails for all
6. Upload all to R2
7. Update the JSON files with real CDN URLs

Expected runtime: 30-60 minutes (depends on Replicate queue)
Expected cost: ~$2.10 (550+ images at $0.003/image + buffer)
...
- Hero images exist for all 17 FloralSource values in honeys/heroes/floral/
- Hero images exist for all 16 HoneyOrigin values in honeys/heroes/origin/
  </action>
  <verify>
...
Run: `echo "Checking hero images exist:" && for origin in USA NEW_ZEALAND AUSTRALIA ARGENTINA MEXICO CANADA BRAZIL GREECE TURKEY SPAIN FRANCE ITALY HUNGARY GERMANY UK OTHER; do echo -n "$origin: "; curl -s -o /dev/null -w "%{http_code}" "${R2_PUBLIC_URL}/honeys/heroes/origin/${origin}.webp"; echo; done`
Verify all origins return 200.
  </verify>
  <done>All seed data images generated including origin region heroes, uploaded to R2, and JSON files updated with permanent CDN URLs</done>
</task>
...
2. `scripts/generate-images.js` implements the full workflow including origin heroes
...
6. Hero images exist for all 17 FloralSource values
7. Hero images exist for all 16 HoneyOrigin values
8. Fly.io has R2 secrets configured
9. Total image count matches expected (~550 images generated: 200 honeys + 50 sources + 17 floral heroes + 16 origin heroes + thumbnails)
</verification>
...
- Hero images generated for each FloralSource type (17 images)
- Hero images generated for each HoneyOrigin region (16 images)
