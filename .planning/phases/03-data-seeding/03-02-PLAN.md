...
- Variety in type (mostly RAW, some CREAMED, occasional COMB and INFUSED)
  ...
  <verify>
Run: `cd /home/sam/claude-workspace/honey-explorer/backend && cat src/main/resources/seed-data/honeys.json | python3 -c "import json,sys; data=json.load(sys.stdin); from collections import Counter; sources=Counter(h.get('floralSource') for h in data); print(f'Total honeys: {len(data)}'); print('FloralSource distribution:'); [print(f'  {k}: {v}') for k,v in sorted(sources.items())]" 2>/dev/null || echo "JSON not valid"`
Verify output shows 200+ honeys total, with distribution matching targets (CLOVER 20+, WILDFLOWER 25+, MANUKA 20+, etc.).
  </verify>
  <done>honeys.json contains 200+ curated honey varieties with complete metadata and proper FloralSource distribution</done>
</task>
...
3. honeys.json has proper FloralSource distribution (each type has minimum count per targets)
4. local-sources.json parses as valid JSON with 50+ entries
5. Each honey has all required fields (floralSource, type, origin, name, slug)
6. Each local source has coordinates and address
7. Application starts with seed.data.enabled=false (no seeding)
8. Application would seed data when seed.data.enabled=true and database is empty
</verification>
...
- honeys.json contains 200+ entries covering all FloralSource values with target distribution
